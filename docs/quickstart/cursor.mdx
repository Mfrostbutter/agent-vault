---
title: "Cursor"
description: "Learn how to connect Cursor to Agent Vault so it can make authenticated API requests without ever seeing your credentials."
---

## Prerequisites

To get the most out of this guide, you'll need to:

- A deployed Agent Vault instance ([see deployment options](/))
- Have [Cursor](https://www.cursor.com) installed

## 1. Log in

```bash
agent-vault login
```

## 2. Create an agent invite

```bash
agent-vault vault agent invite create
```

This generates a one-time onboarding prompt and copies it to your clipboard.

## 3. Paste the prompt into Cursor

Open Cursor's agent chat and paste the invite prompt. Cursor redeems the invite automatically and receives a session token scoped to your vault.

## 4. Start using it

Ask Cursor to do something that requires an external API:

> "Fetch my recent Stripe charges"

Here's what happens — with zero pre-configuration:

1. Cursor routes a request to `api.stripe.com` through Agent Vault
2. Agent Vault returns a **403** — Stripe isn't configured yet
3. Cursor reads the `proposal_hint` and creates a proposal proposing Stripe access
4. Cursor presents you with an approval link
5. You click the link, paste your Stripe key, and click **Allow**
6. Cursor retries — Agent Vault reconstructs the request with your Stripe credentials and the request succeeds

<Note>
You didn't pre-configure anything. The agent discovered what it needed, asked for approval, and handled the rest.
</Note>

## Next steps

<CardGroup cols={2}>
  <Card title="How it works" icon="lightbulb" href="/how-it-works">
    Full request lifecycle end-to-end.
  </Card>
  <Card title="Policies" icon="shield" href="/configuration/policies">
    Pre-configure services you know the agent will need.
  </Card>
</CardGroup>
