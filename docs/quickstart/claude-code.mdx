---
title: "Claude Code"
description: "Learn how to connect Claude Code to Agent Vault so it can make authenticated API requests without ever seeing your credentials."
---

## Prerequisites

To get the most out of this guide, you'll need to:

- A deployed Agent Vault instance ([see deployment options](/))
- Have [Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview) installed

## 1. Log in

```bash
agent-vault login
```

## 2. Create an agent invite

```bash
agent-vault vault agent invite create
```

This generates a one-time onboarding prompt and copies it to your clipboard.

## 3. Paste the prompt into Claude Code

Open Claude Code and paste the invite prompt. Claude Code redeems the invite automatically and receives a session token scoped to your vault.

## 4. Start using it

Ask Claude Code to do something that requires an external API:

> "Fetch my recent Stripe charges"

Here's what happens — with zero pre-configuration:

1. Claude Code routes a request to `api.stripe.com` through Agent Vault
2. Agent Vault returns a **403** — Stripe isn't configured yet
3. Claude Code reads the `proposal_hint` and creates a proposal proposing Stripe access
4. Claude Code presents you with an approval link
5. You click the link, paste your Stripe key, and click **Allow**
6. Claude Code retries — Agent Vault reconstructs the request with your Stripe credentials and the request succeeds

<Note>
You didn't pre-configure anything. The agent discovered what it needed, asked for approval, and handled the rest.
</Note>

## Next steps

<CardGroup cols={2}>
  <Card title="How it works" icon="lightbulb" href="/how-it-works">
    Full request lifecycle end-to-end.
  </Card>
  <Card title="Policies" icon="shield" href="/configuration/policies">
    Pre-configure services you know the agent will need.
  </Card>
</CardGroup>
