---
title: "Proposals"
description: "Review, approve, and manage agent-initiated credential and policy changes."
---

A `Proposal` is a request from an agent to add, update, or remove [credentials](/learn/credentials) and host rules within a specific [vault](/learn/vaults).
...
- **Rules**: Host access changes that define which services the [agent](/agents/overview) wants to reach and how Agent Vault should authenticate to them. These merge into the [vault's](/learn/vaults) [policy](/learn/policies) on approval. For example, a rule might grant access to `api.stripe.com` using a bearer token.
...
Approval atomically merges rules into the [vault's](/learn/vaults) [policy](/learn/policies) and upserts/deletes [credentials](/learn/credentials) in a single transaction.
  ...
  <Tab title="Browser">
    ...
    <Steps>
      ...
      <Step title="Provide credentials and approve">
        Fill in any credential values the agent requested, review the proposed rules, and click **Allow**.
      </Step>
    </Steps>
...
Each rule and credential slot carries an `action` field:

- `set`: idempotent upsert (add or replace). Rules require `host` + `auth`; credential slots require `key`.
- `delete`: remove an existing entry. Rules only need `host`; credential slots only need `key`.
...
Every credential key referenced in a rule's `auth` config must resolve to either a credential slot in the proposal or an existing [credential](/learn/credentials) in the [vault](/learn/vaults). Credential keys use `UPPER_SNAKE_CASE` format.

### Limits

| Constraint                      | Value       |
| ------------------------------- | ----------- |
| Max rules per proposal          | 10          |
