...
A `Vault` is a secure logical container for credentials with policies for how agents can proxy requests through it.
...
- **[Policies](/learn/policies)**: Rules for which hosts (e.g. `api.stripe.com`) can be accessed through the vault along with which and how credential(s) must be attached onto each proxy request.
...
- **[Proposals](/learn/proposals)**: Requests from agents to set credential(s) and/or add host rule(s) that use those credentials.
...
<Warning>
  Deleting a vault permanently removes all its [policies](/learn/policies),
...
Owners see every vault in their vault list. Vaults they have not joined appear in a separate "Other Vaults" section with a **Join** button. Joining grants the owner admin access, letting them manage credentials, approve proposals, and configure policies.
