...
All proxied requests are forwarded over **HTTPS**, regardless of how the agent connects to Agent Vault locally. The agent's `Authorization` header (containing its Agent Vault session token) is stripped before forwarding; real credentials are injected server-side based on the vault's [policy](/learn/policies).
...
- **Instance roles** (`owner`, `member`) control who can manage users, vaults, and server settings. Owners do not get implicit vault access.
